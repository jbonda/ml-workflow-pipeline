...
class DMM(ModelSelection):
    def split_data(self, test_size):
        ...
        if self.data is not None:
            ...
            if (
                not self.x.apply(np.isreal).all().all()
                or not self.y.apply(np.isreal).all().all()
            ):
                flash(
                    "Selected input and/or target column(s) must be numeric.", "danger"
                )
                return None, None, None, None
            ...
            if test_size == 0:
                flash(
                    "For optimal models, please enter a non-zero test size.", "warning"
                )
                return None, None, None, None
            if test_size >= 0.5:
                flash(
                    "For optimal models, please enter a valid test size less than 0.5.",
                    "warning",
                )
                return None, None, None, None
